...
// updateMap updates the cumulative coverage map with the most recent request's coverage
// and keeps track of the delta coverage in a map
	...
	for newFilename, newLineCount := range newCoverage {
		...
		if oldLineCount, ok := coverage.Map[newFilename]; ok {
			...
			for i := range newLineCount {
				// This is unreachable
				if newLineCount[i] == nil {
					deltaLineCount[i] = nil
					continue
				}
...
func (coverage *Coverage) ReadCoverage() error {
	// Coverage is a json mapping of filename (string) to an array of int pointers,
	// where each element of the array indicates the line and the amount of coverage.
	// Nil indicates the line is unreachable (i.e. whitespace, comment)
	...
	// Update coverage map
	deltaMap := coverage.updateMap(dst)
